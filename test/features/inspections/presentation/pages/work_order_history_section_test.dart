...
void main() {
  testWidgets('embedded history excludes drafts and other OS, expands and reacts to sync', (tester) async {
    tester.view.physicalSize = const Size(800, 1600);
    tester.view.devicePixelRatio = 1;
    addTearDown(tester.view.resetPhysicalSize);
    addTearDown(tester.view.resetDevicePixelRatio);
    Inspection item(String id, String os, InspectionSyncStatus status) => Inspection(
      clientId: id, workOrderId: os, syncStatus: status,
      observation: 'Observação completa da inspeção $id.',
      latitude: -7.123, longitude: -34.845,
      photoPath: 'missing-photo.jpg',
      capturedAt: DateTime(2026, 9, 8, 12),
      createdAt: DateTime(2026, 9, 7), updatedAt: DateTime(2026, 9, 10),
    );
    final completed = item('completed', 'wo_1', InspectionSyncStatus.pending);
    registerFallbackValue(completed);
    final items = [completed, item('draft', 'wo_1', InspectionSyncStatus.draft), item('other', 'wo_2', InspectionSyncStatus.synced)];
    ...
    when(() => local.getInspectionByClientId('completed')).thenAnswer((_) => items.first);
    when(() => local.saveInspection(any())).thenAnswer((call) async {
      items[0] = call.positionalArguments.first as Inspection;
    });
    when(() => remote.submitInspection(any())).thenAnswer((_) async => 'server-id');
    final repository = InspectionsRepository(localDataSource: local, remoteDataSource: remote,
      photoService: MockInspectionPhotoService(), locationService: MockInspectionLocationService());
    await tester.pumpWidget(RepositoryProvider.value(value: repository,
      child: const MaterialApp(home: Scaffold(body: SingleChildScrollView(child: WorkOrderHistorySection(workOrderId: 'wo_1'))))));
    ...
    expect(find.text('Observação completa da inspeção completed.'), findsNothing);
    await tester.tap(find.byType(ExpansionTile));
    await tester.pumpAndSettle();
    expect(find.text('Observação completa da inspeção completed.'), findsOneWidget);
    ...
    await tester.pumpAndSettle();
    expect(find.byType(InspectionHistoryCard), findsNothing);
    expect(tester.widget<ChoiceChip>(find.widgetWithText(ChoiceChip, 'Pendentes')).selected, isTrue);
